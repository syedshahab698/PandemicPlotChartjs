...
def predict():
    ...
    t, y1,y2,y3,title = SIR_Model(beta,gamma)
    
    return render_template("predict.html",xlabel=list(t) , y1=list(y1),y2=list(y2),y3=list(y3),title=str(title))#time = time)
...
def SIR_Model(Beta,Gamma):
    ...
    # plt.close()
    # return time
    return t, S/1000, I/1000, R/1000,title
